...
export const ProductCardTemplate = ({ props, resources, resumeContext }) => {
  ...
  if (productState.status === 'pending') {
    ...
    const edgeSlot = resumeContext.boundary?.id
      ? `<small data-edge-slot="${escapeHtml(resumeContext.boundary.id)}">edge segment: waiting for inferred closure-safe data</small>`
      : '';
    const originSlot = resumeContext.boundary?.id
      ? `<small data-origin-slot="${escapeHtml(resumeContext.boundary.id)}">origin resource: waiting for generated async data</small>`
      : '';
