...
import { defaultApp, demoApps, getDemoApp } from '../app/registry.js';
...
const startServer = (port) => {
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === ROUTES.productPartial) {
      ...
      if (url.pathname === ROUTES.edgeSegmentPartial) {
      ...
      if (url.pathname === ROUTES.gallery) {
        res.writeHead(200, {
          'content-type': 'text/html; charset=utf-8',
          'cache-control': 'no-store',
        });
      ...
      const slug = appSlugFromPathname(url.pathname);
