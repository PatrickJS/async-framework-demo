...
import { handleMiniWebDemoRequest } from '../src/browser/miniweb.js';
...
const gallery = await readText('/');
assert.match(gallery, /Async Framework Demo Gallery/);
assert.match(gallery, /href="\/product-cache"/);
assert.match(gallery, /Reset service worker/);

const projectGallery = await readText('/async-framework-demo/');
assert.match(projectGallery, /Async Framework Demo Gallery/);
assert.match(projectGallery, /href="\/async-framework-demo\/product-cache"/);

const debug = await readText('/debug');
assert.match(debug, /Service Worker Debug Harness/);
assert.match(debug, /id="run-debug"/);
assert.match(debug, /\/_async\/partial\/ProductCard/);
assert.match(debug, /async-framework-demo:version/);

const projectDebug = await readText('/async-framework-demo/debug');
assert.match(projectDebug, /\/async-framework-demo\/_async\/partial\/ProductCard/);

const page = await readText('/product-cache?mode=wait&cache=component&delay=0&clear=1');
...
const projectPage = await readText('/async-framework-demo/product-cache?runtime=iframe&mode=wait&cache=component&delay=0&clear=1');
assert.match(projectPage, /MiniWeb runtime<\/strong><br>iframe/);
assert.match(projectPage, /action="\/async-framework-demo\/product-cache"/);

const projectFetchPage = await readText('/async-framework-demo/product-cache?runtime=iframe&mode=fetch&cache=component&delay=0&clear=1');
assert.match(projectFetchPage, /\/async-framework-demo\/_async\/partial\/ProductCard/);

const partialResponse = await handleMiniWebDemoRequest(request(
  '/_async/partial/ProductCard?app=component-partials&id=pending-ProductCardTemplate-1&productId=1&cache=request&store=memory&segment=free&delay=0',
));
...
const iframePartialResponse = await handleMiniWebDemoRequest(request(
  '/_async/partial/ProductCard?runtime=iframe&app=component-partials&id=pending-ProductCardTemplate-1&productId=1&cache=request&store=memory&segment=free&delay=0',
));
...
const edgeResponse = await handleMiniWebDemoRequest(request(
  '/_async/partial/edge-segment?app=segment-vary&id=pending-ProductCardTemplate-1&productId=1&cache=request&store=memory&segment=pro&delay=0',
));
...
const stream = await readText(
  '/streaming?mode=stream&cache=request&state=pending&ssr=pending&delay=0&ids=1,2&delays=1:0,2:0&clear=1',
);
...
const reset = await readText('/?nosw=1');
